...
// Fonction pour télécharger une image avec timeout court et retry
function downloadImage(url, filepath, retries = 2) {
  return new Promise((resolve, reject) => {
    const protocol = url.startsWith('https') ? https : http;

    console.log(`   📥 Tentative de téléchargement (${3 - retries}/3)...`);
    ...
    request.on('error', (err) => {
      if (retries > 0) {
        console.log(`   ⚠️  Erreur: ${err.message}, nouvelle tentative...`);
        setTimeout(() => {
    ...
    // Timeout plus court: 10 secondes
    request.setTimeout(10000, () => {
      request.destroy();
      if (retries > 0) {
        console.log(`   ⏱️  Timeout, nouvelle tentative...`);
        setTimeout(() => {
          downloadImage(url, filepath, retries - 1)
            .then(resolve)
            .catch(reject);
...
async function migrateImages() {
  console.log('🚀 Migration des images restantes vers Supabase Storage...\n');
  ...
  fs.writeFileSync(
    'migration-remaining-report.json',
    JSON.stringify({
  ...
  console.log('\n💾 Rapport de migration sauvegardé: migration-remaining-report.json');

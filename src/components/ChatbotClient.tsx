...
const ChatbotClient: React.FC = () => {
  ...
  const [isOpen, setIsOpen] = useState(() => {
    const saved = localStorage.getItem('chatbot_client_isOpen');
    return saved ? JSON.parse(saved) : false;
  });
  const [isMinimized, setIsMinimized] = useState(() => {
    const saved = localStorage.getItem('chatbot_client_isMinimized');
    return saved ? JSON.parse(saved) : false;
  });
  const [messages, setMessages] = useState<Message[]>(() => {
    const saved = localStorage.getItem('chatbot_client_messages');
    if (saved) {
      try {
    ...
    return [
      {
        role: 'assistant',
        content: 'Bonjour ! 😊 Bienvenue chez Au Matin Vert, votre épicerie de quartier ! Je suis là pour vous aider à découvrir nos produits frais, vous donner des idées de recettes, ou répondre à toutes vos questions. Qu\'est-ce qui vous ferait plaisir aujourd\'hui ? 🥕🍎✨',
        timestamp: new Date()
      }
    ];
  });
  ...
  useEffect(() => {
    localStorage.setItem('chatbot_client_isOpen', JSON.stringify(isOpen));
  }, [isOpen]);

  // Sauvegarder l'état isMinimized dans localStorage
  useEffect(() => {
    localStorage.setItem('chatbot_client_isMinimized', JSON.stringify(isMinimized));
  }, [isMinimized]);

  // Sauvegarder les messages dans localStorage
  useEffect(() => {
    localStorage.setItem('chatbot_client_messages', JSON.stringify(messages));
  }, [messages]);
  ...
  const sendMessage = async () => {
    ...
    if (!apiKey) {
      toast.error('Clé API non configurée. Veuillez contacter l\'administrateur.');
      return;
    }
    ...
    try {
      // Appel à l'Edge Function CLIENT
      const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
      const supabaseKey = import.meta.env.VITE_SUPABASE_ANON_KEY;

      const response = await fetch(`${supabaseUrl}/functions/v1/chatbot-client`, {
        method: 'POST',
        headers: {
  ...
  if (isLoadingApiKey || !apiKey || !isChatbotEnabled || isAdminPage) {
    return null;
  }
      ...
      <div className="fixed right-0 top-1/2 -translate-y-1/2" style={{ zIndex: 9999 }}>
        ...
        <div className="relative group">
          ...
          <button
            ...
            title="Ouvrir l'assistant Au Matin Vert"
            style={{
              minWidth: '60px',
    ...
    return (
      <div className="fixed bottom-6 right-6" style={{ zIndex: 9999 }}>
        ...
        >
          ...
          <span className="font-medium">Assistant Au Matin Vert</span>
          {messages.length > 1 && (
    ...
    <div className="fixed bottom-6 right-6 w-96 h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col border border-zinc-200" style={{ zIndex: 9999 }}>
      ...
      <div className="p-4 rounded-t-2xl flex items-center justify-between" style={{
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-white">Assistant Au Matin Vert</h3>
            <p className="text-xs text-white/70">Votre épicerie de quartier</p>
          </div>
        </div>

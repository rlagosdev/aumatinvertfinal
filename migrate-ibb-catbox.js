...
// Fonction pour télécharger une image avec User-Agent
function downloadImageWithUA(url, filepath) {
  return new Promise((resolve, reject) => {
    ...
    const request = protocol.get(options, (response) => {
      if (response.statusCode === 301 || response.statusCode === 302) {
        ...
        downloadImageWithUA(response.headers.location, filepath)
          .then(resolve)
          .catch(reject);
        return;
      }
...
async function migrateImages() {
  console.log('🚀 Migration des images ibb.co et catbox.moe vers Supabase Storage...\n');

  // Charger le rapport
  const report = JSON.parse(fs.readFileSync('external-images-report.json', 'utf8'));

  // Filtrer pour ne garder que ibb.co et catbox.moe
  const images = report.images.filter(img =>
    img.url.includes('i.ibb.co') || img.url.includes('files.catbox.moe')
  );

  console.log(`📊 Images à migrer: ${images.length}\n`);
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    try {
      ...
      // Télécharger l'image avec User-Agent
      console.log(`   📥 Téléchargement avec User-Agent...`);
      await downloadImageWithUA(img.url, localPath);
  ...
  fs.writeFileSync(
    'migration-ibb-catbox-report.json',
    JSON.stringify({
  ...
  console.log('\n💾 Rapport de migration sauvegardé: migration-ibb-catbox-report.json');
